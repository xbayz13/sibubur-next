...
export const reportsService = {
  async getDailyReport(date: string, storeId?: number): Promise<DailyReport & { recommendations?: ProductionRecommendation }> {
    const params = storeId ? { storeId } : {};
    const response = await apiClient.get<DailyReport & { recommendations?: ProductionRecommendation }>(
      `/reports/daily/${date}`,
      { params }
    );
  ...
  async getMonthlyReport(year: number, month: number, storeId?: number): Promise<MonthlyReport> {
    const params = storeId ? { storeId } : {};
  ...
  async getYearlyReport(year: number, storeId?: number): Promise<YearlyReport> {
    const params = storeId ? { storeId } : {};
